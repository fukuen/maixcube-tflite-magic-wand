...
TfLiteStatus SetupAccelerometer(tflite::ErrorReporter* error_reporter) {
  ...
  // meet kTargetHz
  IMU.setDataRate(MSA300_DATARATE_250_HZ);
  float sample_rate = 250;
...
bool ReadAccelerometer(tflite::ErrorReporter* error_reporter, float* input,
  ...
  while (!new_data) {
    ...
    acc_t acceleration = IMU.getAcceleration();
    x = acceleration.x;
    y = acceleration.y;
    z = acceleration.z;
    ...
    const float norm_z = y;
    save_data[begin_index++] = norm_x * 1000 / GRAVITY;
    save_data[begin_index++] = norm_y * 1000 / GRAVITY;
    save_data[begin_index++] = norm_z * 1000 / GRAVITY;
